...
JSON_INPUT_FILTERED = os.path.join(BASE_DIR, "input_sampled_fsq_json", "input_filtered.json")
JSON_INPUT_ALL = os.path.join(BASE_DIR, "input_sampled_fsq_json", "input_all_categories.json")
JSON_OUTPUT_FILTERED = os.path.join(BASE_DIR, "output_syn_json", "fsq_to_synthetic_filtered.json")
JSON_OUTPUT_ALL = os.path.join(BASE_DIR, "output_syn_json", "fsq_to_synthetic_all_categories.json")
...
def assign_transactions_only(visits):
    """
    Direct translation: Convert ALL FSQ checkins to transactions only.
    No views, no reviews - just transaction data with placeholders for the rest.
    
    Each checkin becomes a transaction directly.
    """
    n_visits = len(visits)
...
def process_input_to_synthetic(input_file, output_file, dataset_name):
    """
    Process one input JSON file and generate corresponding synthetic dataset
    """
    print(f"\n{'='*60}")
