...
def process_input_file(input_file, poi_mapping, cluster_summary, user_to_cluster):
    ...
    for user_id, checkins in user_data.items():
        ...
        # Cluster Metadata - FIXED
        cluster_id = user_to_cluster.get(user_id, "Unknown")
        
        # Try to get cluster info with better error handling
        cluster_info = {}
        if cluster_id != "Unknown":
            try:
                # Convert cluster_id to int if it's a string
                cluster_key = int(cluster_id) if isinstance(cluster_id, str) else cluster_id
                cluster_info = cluster_summary.get(cluster_key, {})
                
                if not cluster_info:
                    missing_clusters.add(cluster_key)
                    
            except (ValueError, KeyError) as e:
                print(f"Warning: Could not find cluster info for cluster_id={cluster_id}, user={user_id}: {e}")
                missing_clusters.add(cluster_id)
        
        # Extract cluster information with defaults
        top_poi_categories = cluster_info.get("top_poi_categories", "Unknown")
        most_active_time = cluster_info.get("hour_peak", "Unknown")
        most_active_day = cluster_info.get("day_peak", "Unknown")
        mean_planning_area = cluster_info.get("planning_area", "Unknown")
        ...
        # Probable User Profile Tag
        probable_tags = []
        if isinstance(top_poi_categories, str):
            top_poi_categories_list = top_poi_categories.split(", ")
            if "Office" in top_poi_categories_list:
                probable_tags.append("Office employees")
            if "Flea Market" in top_poi_categories_list:
    ...
    if missing_clusters:
        print(f"\nWarning: {len(missing_clusters)} cluster IDs not found in cluster_summary:")
        print(f"Missing cluster IDs: {sorted(list(missing_clusters))[:20]}...")
