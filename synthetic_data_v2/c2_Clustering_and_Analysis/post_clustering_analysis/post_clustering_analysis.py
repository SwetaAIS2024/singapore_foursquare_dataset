...
from c0_Configuration.config_paths import CATEGORIES_XLSX, FINAL_INPUT_DATASET, CLUSTER_OUTPUT_DIR, PLACE_ID_POI_CAT, CHECKINS_PATH
from c1_Data_Collection_and_Processing.util import load_config


POST_CLUSTER_ANALYSIS_OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output")
...
def Post_Clustering():
    try:
        ...
        for algo_name in algos_to_analyze:
            ...
            tsv_file_path = os.path.join(CLUSTER_OUTPUT_DIR, algo_name, "user_cluster_labels_user_to_cluster.txt")
            if not os.path.exists(tsv_file_path):
                print(f"[WARNING] TSV file not found for {algo_name}: {tsv_file_path}")
                continue
            
            # Load directly from TSV
            user_cluster_df = pd.read_csv(tsv_file_path, sep='\t', names=['user_id', 'cluster'], dtype={'user_id': int, 'cluster': int})
            user_ids = user_cluster_df['user_id'].tolist()
            labels = user_cluster_df['cluster'].tolist()
