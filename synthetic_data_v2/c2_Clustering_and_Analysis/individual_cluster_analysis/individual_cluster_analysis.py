...
import gc
import warnings
from c0_Configuration.config_paths import CLUSTER_OUTPUT_DIR, FINAL_INPUT_DATASET
...
def process_cluster(cluster_id, processed_vectors, labels, output_dir, scaled_rss_threshold=5, algo_name=None):
    ...
    for dim_idx, dim in enumerate(significant_dims):
        ...
        distrs = ['norm', 'expon', 'uniform', 'cauchy', 'laplace', 'genextreme', 't', 'loggamma', 'dweibull']
        ...
        try:
            ...
            dist_name = best_row['name']
            # Convert params to list if needed
            if isinstance(params, dict):
                params_list = list(params.values())
            else:
                params_list = list(params)
            # For distributions that require scale > 0
            if dist_name in ['t', 'norm', 'laplace', 'genextreme', 'dweibull', 'cauchy', 'loggamma']:
                scale = params_list[-1]
                if scale <= 0:
                    print(f"[WARNING] Skipping dimension {dim}: invalid scale parameter ({scale}) for {dist_name}")
                    continue
            # ---------------------------------
    ...
    fitted_params = {}
    for dim_fit in dimension_fits:
        dim = dim_fit['dimension']
        dist_name = dim_fit['distribution']['name']
        params = dim_fit['distribution']['parameters']
        fitted_params[dim] = {'distribution': dist_name, 'parameters': params}
    
    # Save fitted parameters
    with open(os.path.join(cluster_dir, 'fitted_parameters.json'), 'w') as f:
...
def analyze_clusters():
    ...
    for algo_name in algos_to_analyze:
        ...
        try:
            ...
            output_dir = os.path.join(os.path.dirname(CLUSTER_OUTPUT_DIR), 'individual_cluster_analysis', 'cluster_analysis', algo_name)
