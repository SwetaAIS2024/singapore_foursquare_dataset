...
data = np.load('analysis_older_dataset/analysis_set4/codes/C_time_matrix/user_category_time_matrices.npy', allow_pickle=True).item()
...
# Save embeddings
pd.DataFrame(user_embeds, index=user_ids).to_csv('analysis_older_dataset/analysis_set4/codes/Dimension_reduction_graph_construction/embeddings/user_pca_embeddings.csv')
...
os.makedirs('analysis_older_dataset/analysis_set4/codes/Dimension_reduction_graph_construction/graph', exist_ok=True)
with open('analysis_older_dataset/analysis_set4/codes/Dimension_reduction_graph_construction/graph/user_similarity_knn_graph.gpickle', 'wb') as f:
    pickle.dump(G, f)
...
nx.draw_networkx_nodes(subG, pos, node_size=50, node_color='blue', alpha=0.7)
nx.draw_networkx_edges(subG, pos, alpha=0.3)
plt.title('User Similarity kNN Subgraph (50 users, 2D)')
plt.axis('off')
plt.tight_layout()
plt.savefig('analysis_older_dataset/analysis_set4/codes/Dimension_reduction_graph_construction/graph/user_similarity_knn_subgraph_2d.png')
plt.show()
...
ys = node_pos_3d[:, 2]
zs = node_pos_3d[:, 1]
ax.scatter(xs, ys, zs, s=50, c='blue', alpha=0.7)
# Draw edges
...
ax.set_title('User Similarity kNN Subgraph (50 users, 3D)')
plt.tight_layout()
plt.savefig('analysis_older_dataset/analysis_set4/codes/Dimension_reduction_graph_construction/graph/user_similarity_knn_subgraph_3d.png')
plt.show()
