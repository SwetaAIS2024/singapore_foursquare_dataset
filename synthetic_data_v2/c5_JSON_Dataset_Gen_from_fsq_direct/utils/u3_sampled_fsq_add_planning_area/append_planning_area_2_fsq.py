...
from c0_Configuration.config_paths import CHECKINS_PATH, SAMPLED_FSQ_PLANNING_AREA, PLANNING_CACHE, SUBZONE_CACHE

# Local cache paths
INPUT_FILE = CHECKINS_PATH
OUTPUT_FILE = SAMPLED_FSQ_PLANNING_AREA
...
def append_planning_area(input_file: str, output_file: str, gdf):
    """
    Read the input file, append the planning area for each row, remove rows without planning area, and save to the output file.
    """
    # Load the input dataset - TSV file WITHOUT headers
    # Columns: user_id, place_id, datetime, timezone, lat, lon
    df = pd.read_csv(input_file, sep="\t", header=None, 
                     names=['user_id', 'place_id', 'datetime', 'timezone', 'lat', 'lon'])
    ...
    if none_count > 0:
        print(f"\nSample rows without planning area (will be removed):")
        no_area = df[df['planning_area'].isna()].head(5)
        print(no_area[['user_id', 'place_id', 'lat', 'lon', 'planning_area']])
    
    # REMOVE ROWS WITHOUT PLANNING AREA
    print(f"\n{'='*60}")
    print(f"CLEANING DATA")
    print(f"{'='*60}")
    initial_count = len(df)
    df_cleaned = df[df['planning_area'].notna()].copy()
    removed_count = initial_count - len(df_cleaned)
    
    print(f"Removed {removed_count:,} rows without planning area")
    print(f"Retained {len(df_cleaned):,} rows ({len(df_cleaned)/initial_count*100:.2f}% of original data)")
    ...
    print(f"All rows have valid planning areas: {df_cleaned['planning_area'].notna().all()}")
    
    # Show distribution
    print(f"\nPlanning Area Distribution (Top 15):")
    area_counts = df_cleaned['planning_area'].value_counts()
    for idx, (area, count) in enumerate(area_counts.head(15).items(), 1):
        pct = count / len(df_cleaned) * 100
        print(f"{idx:2d}. {area:<20} {count:>6,} ({pct:>5.2f}%)")
    
    # Additional statistics
    print(f"\nDataset Statistics:")
    print(f"Number of unique planning areas: {df_cleaned['planning_area'].nunique()}")
    ...
    print(f"✅ Saved cleaned dataset with planning areas")
