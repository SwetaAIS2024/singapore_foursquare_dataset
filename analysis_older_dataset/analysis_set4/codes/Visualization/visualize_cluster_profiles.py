...
with open('analysis_older_dataset/analysis_set4/codes/Clustering_Profiling/final_cluster_profiles_v2.json') as f:
...
sum_df.to_csv('analysis_older_dataset/analysis_set4/codes/Visualization/cluster_summary_table.csv', index=False)
...
for cid, prof in profiles.items():
    ...
    plt.ylabel('Rank')
    plt.tight_layout()
    plt.savefig(f'analysis_older_dataset/analysis_set4/codes/Visualization/cluster_{cid}_poi_pref_bar.png')
    plt.close()
...
for cid, prof in profiles.items():
    ...
    plt.tight_layout()
    plt.savefig(f'analysis_older_dataset/analysis_set4/codes/Visualization/cluster_{cid}_timewise_bar.png')
    plt.close()

# --- 3D Plot: Time Bracket x Dominant POI x Cluster ---
# Load summary table
csv_path = 'analysis_older_dataset/analysis_set4/codes/Visualization/cluster_summary_table.csv'
...
plt.tight_layout()
plt.savefig('analysis_older_dataset/analysis_set4/codes/Visualization/cluster_3d_time_poi_cluster.png')
plt.show()
# --- End 3D Plot ---

print('Visualization complete. See analysis_older_dataset/analysis_set4/codes/Visualization for results.')
