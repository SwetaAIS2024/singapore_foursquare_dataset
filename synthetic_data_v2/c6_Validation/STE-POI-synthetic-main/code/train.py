...
if __name__ == '__main__':
    ...
    train_data, val_data, test_data = process_user_histories(user_data, args.train_ratio, args.val_ratio)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_no, batch in enumerate(train_loader):
            ...
            loss_cat = criterion_cls(cat_logits, target_cat)
        ...
        # val_metric = evaluate(model, val_loader, full_poi_embeds, args.device)
        # val_metric = evaluate_length_bucket(model, val_loader, full_poi_embeds, args.device)
        if val_metric > best_val_score:
            print("\n=== Best Validation Result ===")
            torch.save(model.state_dict(), f'../saved_model/recommendation_model_{epoch}.pth')
            best_val_score = val_metric
            
            
            print(f"Run model for test data")
            # test_metrics = evaluate_test(model, test_loader, full_poi_embeds, poi_idxs, poi_cat, poi_cont, poi_coords, args.device)
            # test_metric = evaluate(model, test_loader, full_poi_embeds, args.device)
            val_metric = evaluate_nxt(model, test_loader, args.device)
            # test_metric = evaluate_length_bucket(model, test_loader, full_poi_embeds, args.device)
